import React from 'react'
import { Link } from "react-router-dom";
import { useEffect, useState } from 'react';
const Home = () => {
    const [shows, setShows] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchShows = async () => {
      fetch("https://api.tvmaze.com/search/shows?q=all").then((response) => {
        return response.json()
      })
        .then((jsonData) => {
          setShows(jsonData)
          setLoading(false)
        })
    };
    fetchShows();
  }, []);

  if (loading) {
    return <div>Loading...</div>;
  }


  const renderImage = (show) => {
    if (show.show.image) {
      return <img src={show.show.image.medium} alt={show.show.name} />;
    } else {
      return <img src="data:image/png;base64," alt="default" />;
    }
  };
  return (
    <div>
        <div className='container App'>
      <h1>TV Shows</h1>
      <ul className='list_style'>
        {shows.map((show) => (

          <div className="card my-3 shadow-lg p-3 mb-5 bg-white rounded card_style" style={{ width: "50rem" }}>
            <div className="card-body">
              {renderImage(show)}
              <h5 className="card-title">{show.show.name}</h5>
              <p className="card-text">Language :{show.show.language}</p>
              <a href={show.show.url} className="card-text"><small>Link to the show</small></a>
              <p className='card-text'>Run Time: {show.show.averageRuntime}</p>
              <p className='card-text'>ID: {show.show.id}</p>
              <Link to={`/summary/${show.show.id}`} className="btn btn-sm btn-dark" >Summary</Link>
            </div>
          </div>
        ))}
      </ul>
    </div>
    </div>
  )
}

export default Home